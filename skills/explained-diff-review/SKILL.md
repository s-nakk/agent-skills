...
- `answer_submit_review`: partial review state only. Do not modify files, approve the snapshot, or continue the workflow. Answer the submitted comments, keep the partial approvals in the conversation, and regenerate the same snapshot screen with `--approved-files` (or `--approved` for whole groups) when another user decision is needed, and add `--exclude-approved` so the approved files leave the screen and the reader sees only what is still open. Do the same when a snapshot goes stale after a partial approval: carry the approvals of the files whose diff did not change into the new build, and leave any file whose diff changed unapproved.
